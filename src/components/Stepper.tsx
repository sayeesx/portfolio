...
export default function Stepper({
  ...
  const handleNext = () => {
    if (!isLastStep) {
      ...
      const isValid = (rest as any).onValidateStep?.() ?? true;
      if (isValid) {
        setDirection(1);
        updateStep(currentStep + 1);
      }
  ...
  const handleComplete = () => {
    // Don't immediately complete - let parent handle completion
    onFinalStepCompleted();
  };
          ...
          <div className={`px-8 pb-8 ${footerClassName}`}>
            <div className={`mt-10 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
              ...
              <button
                ...
                className="duration-350 flex items-center justify-center rounded-full bg-white py-1.5 px-3.5 font-medium tracking-tight text-black transition hover:bg-gray-200 active:bg-gray-300"
                {...nextButtonProps}
              >
                {nextButtonProps.children || (isLastStep ? 'Send' : nextButtonText)}
              </button>
            </div>
          </div>
        )}
